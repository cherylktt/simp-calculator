...
object Parser {
    ...
    /*  T  ::= X | C | (E)  
        ...
        E' ::= epsilon
    */
    case class ExpLeft(t:Exp, ep:ExpLeftPrime)

    enum ExpLeftPrime {
        case OpExpExpPrime(op:LToken, e:Exp, ep:ExpLeftPrime)
        case Eps
    }

    import ExpLeftPrime.*

    def parse_term:Parser[PEnv,Exp] = choice(parse_x)(choice(parse_c)(parse_paren_exp))

    def parse_x:Parser[PEnv, Exp] = for {
        x <- p_var
    } yield VarExp(x)
    def parse_c:Parser[PEnv, Exp] = for {
        c <- p_const
    } yield ConstExp(c)
    def parse_paren_exp:Parser[PEnv, Exp] = for {
        _ <- p_lparen
        e <- p_exp
        _ <- p_rparen
    } yield ParenExp(e)
    ...
    def parse_exp_prime:Parser[PEnv, ExpLeftPrime] = for {
        opt <- optional(parse_op_exp_exp_prime)
    } yield { opt match {
        case Left(_) => Eps
        case Right(opt) => opt
    }}

    def parse_op_exp_exp_prime:Parser[PEnv, ExpLeftPrime] = for {
        _ <- p_spaces
        op <- p_op
        _ <- p_spaces
        exp <- p_exp
        ep <- parse_exp_prime
    } yield OpExpExpPrime(op, exp, ep)

    // def parse_epsilon:Parser[PEnv, ExpLeftPrime] = Eps

    def fromLEP(t:Exp)(ep:ExpLeftPrime):Exp = ep match {
        case OpExpExpPrime(op, e, ep1) => {
            fromLEP(operation(t)(op)(e))(ep1)
        }
        case Eps => t
    }

    def operation(e1:Exp)(op:LToken)(e2:Exp) = op match {
