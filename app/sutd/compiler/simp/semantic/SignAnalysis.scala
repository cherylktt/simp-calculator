...
object SignAnalysis {
    ...
    // join(s) = U pred(s) => pred is predecessor
    def join(preds:List[AbstractState]):AbstractState = preds match {
      case Nil => Map.empty
      case (x::xs) => xs.foldLeft(x) { // x is the first AbstractState
        (acc, x1) => acc ++ x1.map { // i want the value from x1
          case (k,v) => val lowerUpperBound = acc.get(k) match {
            case Some(a) => signLattice.lub(a,v)
            case None => v
          }
          k -> lowerUpperBound // update acc
        } 
      }
    }
          ...
          foldM(
            (acc:AbstractEnv, li:LabeledInstr) => li match {
              ...
              case (label, IPlus(Temp(AVar(t)), src1, src2)) => {
                val joined_preds_states = joinPredStates(label, acc)
                (getOprSign(joined_preds_states, src1), getOprSign(joined_preds_states, src2)) match {
                  case (Left(err), _) => Left(err)
                  case (_, Left(err)) => Left(err)
                  case (Right(sign1), Right(sign2)) => Right(acc + (label -> 
                    (joined_preds_states + (t -> absPlus(sign1, sign2)))
                  ))
                }
              }
              ...
              case (label, IMinus(Temp(AVar(t)), src1, src2)) => {
                ...
                (getOprSign(joined_preds_states, src1), getOprSign(joined_preds_states, src2)) match {
                  ...
                  case (_, Left(err)) => Left(err)
                  case (Right(sign1), Right(sign2)) => Right(acc + (label -> 
                    (joined_preds_states + (t -> absMinus(sign1, sign2)))
                  ))
                }
              }
              ...
              case (label, IMult(Temp(AVar(t)), src1, src2)) => {
                ...
                (getOprSign(joined_preds_states, src1), getOprSign(joined_preds_states, src2)) match {
                  ...
                  case (_, Left(err)) => Left(err)
                  case (Right(sign1), Right(sign2)) => Right(acc + (label -> 
                    (joined_preds_states + (t -> absMult(sign1, sign2)))
                  ))
                }
              }
              ...
              case (label, IDEqual(Temp(AVar(t)), src1, src2)) => {
                ...
                (getOprSign(joined_preds_states, src1), getOprSign(joined_preds_states, src2)) match {
                  ...
                  case (_, Left(err)) => Left(err)
                  case (Right(sign1), Right(sign2)) => Right(acc + (label -> 
                    (joined_preds_states + (t -> absDEqual(sign1, sign2)))
                  ))
                }
              }
              ...
              case (label, ILThan(Temp(AVar(t)), src1, src2)) => {
                ...
                (getOprSign(joined_preds_states, src1), getOprSign(joined_preds_states, src2)) match {
                  ...
                  case (_, Left(err)) => Left(err)
                  case (Right(sign1), Right(sign2)) => Right(acc + (label -> 
                    (joined_preds_states + (t -> absLThan(sign1, sign2)))
                  ))
                }
              }
