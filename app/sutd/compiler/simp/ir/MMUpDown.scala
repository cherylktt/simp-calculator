    ...
    def genExp(e:Exp)(using me:StateCogenMonad[StateInfo]):CogenState[(Opr, List[(Label,Instr)])] = e match {
        ...
        */ 
        ...
        /*
         GE(e1) |- (up_e1, down_e1)
         GE(e2) |- (up_e2, down_e2)
         X is a fresh var => use newTemp, call Temp(x)
         L is a fresh label => use newName, call Regstr(L)
        --------------------------------------------------------------------- (Op)
         GE(e1 op e2) |- (X, down_e1 ++ down_e2 ++ [L:X <- up_e1 op up_e2])
         */

        // this might work, big might ok nice it works
        case Plus(e1,e2) => for {
            (u1,d1) <- genExp(e1)
            (u2,d2) <- genExp(e2)
            x <- newTemp
