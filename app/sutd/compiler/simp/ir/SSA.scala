...
import sutd.compiler.simp.syntax.Lexer.string
import sutd.compiler.simp.interpreter.PAInt.update
...
object SSA {
    ...
    // pa is original program P (list of labeled instructions)
    // what is E. E is a dictionary that maps labels to set of variables (or list in this case)
    // (l,S) in E implies that every x in S has phi assignments to be inserted at l
    // p is modified program Q ?? i assume
    ...
    def insertPhis(pa:List[LabeledInstr], dft:DFTable, g:CFG):P = {
        ...
        print(labels_modded_vars)
        println()
        
        // Task 1.2 TODO
        // convert List[(Label, List[String])] to Map[Label, List[String]]
        val e:E = {

            // convert List[(Label, List[String])] to Map[String, List[Label]]
            val stringMap:Map[String, List[Label]] = labels_modded_vars.flatMap(
                (label, stringList) => stringList.map(
                    string => (string, label)
                )
            ).foldLeft(
                Map.empty[String, List[Label]]
            ) {
                case (acc, (k, v)) => 
                    val currentVal = acc.getOrElse(k, List())
                    val updateVal = currentVal :+ v
                    acc + (k -> updateVal)
            }
            print(stringMap)
            println()

            // get the new labels from dfPlus in DF.scala
            val dfPlusMap:Map[String, List[Label]] = stringMap.map{
                case (string, labels) => string -> dfPlus(dft, labels)
            } 
            print(dfPlusMap)
            println()

            // go back to the normal format
            val labelMap = dfPlusMap.toList.flatMap(
                (string, labelList) => labelList.map(
                    label => (label, string)
                )
            ).foldLeft(
                Map.empty[Label, List[String]]
            ) {
                case (acc, (k, v)) => 
                    val currentVal = acc.getOrElse(k, List())
                    val updateVal = currentVal :+ v
        ...
        // (l:Label, phis:List[PhiAssignment], i:Instr)
        // PhiAssignment(dest:Opr, operands:List[(Label, AVar)], stem:opr)
        // phis = map (\x -> x <- phi(k:x | (k,l) in G)) xs
        print("watch it fail here")
        println()
        val pa_with_phis:List[SSALabeledInstr] = pa.foldLeft(List():List[SSALabeledInstr])((acc, li:LabeledInstr) => li match {
            case (l, i) => e.get(l) match {
                case None => acc ++ List((l, List(), i))
                case Some(xs) => {
                    // print("acc is")
                    // print(acc)
                    // println()
                    ...
                    val edges = g.filter {
                        case (k, las) => las.contains(l)
                    }.map{case (k,_) => k}.toList
                    print(edges)
                    println()
                    
                    val phis1 = edges match {
                        case List() => List()
                        case _ => xs.map {x => 
                            val oprs = edges.map(k => (k, AVar(x)))
                            PhiAssignment(Temp(AVar(x)), oprs, Temp(AVar(x)))
                        }
                    }
                    print(phis1)
                    println()
        ...
        // convert List[SSALabeledInstr] to Map[Label, SSALabeledInstr]
