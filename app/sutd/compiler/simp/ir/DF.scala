...
object DF {
    ...
    */
    ...
    // Task 1.1 TODO 
    // dfLocal(v, G) = {w  | (v,w) in G and v does not preceed w}
    def dfLocal(v:Label, dt:DomTree, g:CFG):List[Label] = successors(g, v) match {
        case Nil => Nil
        case m => m.toList.foldLeft(List():List[Label])(
            (acc, w) => {acc ++ (isChildOf(w, v, dt) match {
                case true => Nil
                case false => List(w)
            })}
        )
    ...
    // def dfLocal(v:Label, dt:DomTree, g:CFG):List[Label] = dfUp(v, dt, g)
    
    // v1 = idom(v2) if v1 preceeds v2 
    // and there is no v3 such that v3 preceeds v2 and v1 preceeds v3
    // idom is based on CFG

    def strictlyDominate(v1:Label, v2:Label, dt:DomTree):Boolean = {
        if (v1==v2) {false}
        else {
            isChildOf(v2, v1, dt)
        }
    ...
    def checkDominance(l:List[Label], v2:Label, dt:DomTree):List[Label] = l match {
        case Nil => List()
        case v1::rest => if (strictlyDominate(v1, v2, dt)) {List(v1)} else {
            checkDominance(rest, v2, dt)
        }
    }

    def idom(v2:Label, dt:DomTree, g:CFG):Label = {
        val preds = predecessors(g, v2)
        val idomVal = checkDominance(preds, v2, dt) 
        idomVal match {
            case List() => 0
            case _ => idomVal.head
        }
    ...
    def buildDFT(dt:DomTree, g:CFG):DFTable = {
        ...
        def go(acc:DFTable, x:Label):DFTable = {
            ...
            // println()
            // Task 1.1 TODO 
            // dfUp(u, G) = {w | w in df(u) and idom(u) does not preceed w}
            // idom(u) is a successor of w
            // acc is df(u)
            // idom(u, dt, g)
            def dfUp(u:Label):List[Label] = acc.get(u) match {
                case None => Nil
                case Some(value) => value.foldLeft(List():List[Label])(
                    (acc1, w) => {acc1 ++ (isChildOf(w, idom(u, dt, g), dt) match {
                        case true => Nil
                        case false => List(w)
                    })}
                )
            }
            // def dfUp(u:Label):List[Label] = List()
